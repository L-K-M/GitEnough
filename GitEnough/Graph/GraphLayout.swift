...
struct GraphLayout: Equatable {
    ...
    /// strips — row r renders `segmentsByRow[r]` in a row-height canvas of its
    /// own — instead of one history-height Canvas, whose backing store
    /// overflows Core Animation's layer size limit on long histories (a
