...
/// needed).
public struct Repository: Identifiable, Hashable, Codable {
...
public struct RepoSummary: Equatable {
    ...
    public static let unknown = RepoSummary(branch: nil, isDirty: false, ahead: 0, behind: 0, isValid: true)
}
...
public enum SidebarSortOrder: String, CaseIterable, Identifiable {
...
public enum SidebarFilter: String, CaseIterable, Identifiable {
    ...
    public func matches(_ summary: RepoSummary) -> Bool {
        switch self {
...
public final class RepoStore: ObservableObject {
    ...
    @Published public var summaries: [String: RepoSummary] = [:]

    /// path → last-opened timestamp, for the "Recently Opened" sort order.
    @Published public private(set) var lastOpenedAt: [String: TimeInterval] = [:]

    /// Paths of starred repositories — pinned above the unstarred ones in every
    /// sidebar sort order.
    @Published public private(set) var starredPaths: Set<String> = []
    ...
    @discardableResult
    public func register(_ repo: Repository) -> Repository {
    ...
    @discardableResult
    public func addDiscovered(_ urls: [URL]) -> Int {
        var added = 0
        for url in urls {
            let path = url.path
    ...
    /// render regardless).
    public func moveVisible(_ visible: [Repository], fromOffsets: IndexSet, toOffset: Int) {
    ...
    public func isStarred(_ repo: Repository) -> Bool {
    ...
    public static func starredFirst(_ repos: [Repository], starred: Set<String>) -> [Repository] {
    ...
    public func markOpened(_ repo: Repository) {
